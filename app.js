...
const cardPairs = () => {
  ...
  while (pairs.length < 16) {
    ...
    if (!indexes.includes(randIndex)) {
      ...
      pairs.push(
        cardCharacters.hiragana[randIndex],
        cardCharacters.romaji[randIndex]
      );
    }
  }
...
const flipCard = (card) => {
  ...
  if (firstCard === null) {
    ...
    for (let idx = 0; idx < winningPairs.length; idx++) {
      if (
        (winningPairs[idx][0] === firstCardValue &&
          winningPairs[idx][1] === secondCardValue) ||
        (winningPairs[idx][0] === secondCardValue &&
          winningPairs[idx][1] === firstCardValue)
      ) {
        isMatch = true;
        break;
      }
    ...
    if (isMatch) {
      ...
      if (matchedCards === 8) {
        ...
        document.getElementById('gameMessage').textContent =
          'You Won! Great job, keep up the good work!';
